...
function displayWorkProgress(hidden) {
    if (hidden) {
        workProgressBox = {};
        return true;
    }else if (workProgressBox == null && !hidden) {
		...
		*/
		...
		workProgressBox.style.zIndex="999999999999";

		var boxContent = ""

			+ " <div class='loadingBox'>"
			+ " 	<img src=\"/images/mng/loading03.gif\" style=\"width:25%;height:25%\" alt=\"로딩중 이미지\">"
			+ " </div>";
...
/**
 * 작업중 박스 표시
 * displayWorkProgress의 top 값이 변경됨
 */
function displayWorkProgress3(hidden) {
    if (hidden) {
        workProgressBox = {};
        return true;
		...
		*/
		...
		workProgressBox.style.left		= winLeft + "px";
		workProgressBox.style.top		= "100%";//winTop + "px";
