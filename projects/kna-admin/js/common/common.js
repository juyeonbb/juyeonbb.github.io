...
//            domArr[domArr.length-1].search(knownDomsPat)==-1) {
   ...
   **/
	...
	**/
	...
	function fn_getCodeList(cdclsf_id){
		var dataList = "";
		jQuery.ajax({
			url : contextpath + 'common/selectCodeList.do',
			async : false,
			cache : false,
			dataType: 'json',
			data : {
  ...
  function downloadFileByFileidForSearch(fileId, fileIdntfcKey){
     $.fileDownload(contextpath + 'downloadFileByFileid.do?fileid='+fileId + '&file_idntfc_key=' + fileIdntfcKey, {
    ...
    function fn_positionPopUp(postnid){
    	window.open(contextpath + 'mng/postn/postnPop.do?postnid='+postnid ,"", 'top=50,left=50,width=900, height=900 ,scrollbars=yes');
    }
    ...
    */
    ...
    function openBrAcctInfoPopup(){
       var popUpUrl = contextpath + "front/btoc/srvc/brAcctInfoPopup.html";
       $.ajax({
           cache: false
           , url: popUpUrl
           , type: 'GET'
           ...
           , success: function(result) {
        	   openPopup('brAcctInfoPopup',result);
           }
       });
    }

    function gotoInqryPage(user){
		if(user){
			location.href= contextpath + "front/btoc/mypage/Inqry/InqryListForm.html";
		}else{
...
function getSessionStorage(params) {
    for(var key in params) {
...
window.onpageshow = function(e) {
    var searchYn = sessionStorage.getItem('isSearch');
    if(e.persisted || (window.performace && window.performance.navigation.type ==2) || searchYn == 'Y') {
        alert('검색유지 O');
    } else {
        alert('검색유지 X');
